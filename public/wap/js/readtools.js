...
var ReadTools = {
    ...
    SetColor: function (id) {
        ...
        if (ReadTools.colorid != id) Cookie.set('read_colorid', id, 365, '/', '', '');
    ...
    SetFont: function (id) {
        ...
        if (ReadTools.fontid != id) Cookie.set('read_fontid', id, 365, '/', '', '');
        ReadTools.fontid = id;
    ...
    FontSmall: function () {
        if (ReadTools.fontid > 0) {
            ...
            Cookie.set('read_fontid', ReadTools.fontid, 365, '/', '', '');
        }
    },
    FontBig: function () {
...
<input type="hidden" name="act" id="act" value="newpost" />\
    ...
    SaveSet: function () {
        Cookie.set('read_colorid', ReadTools.colorid, 365, '/', '', '');
        Cookie.set('read_fontid', ReadTools.fontid, 365, '/', '', '');
    },
    LoadSet: function () {
        var id = 0;

        id = parseInt(Cookie.get('read_colorid'));
        if (id >= 0 && id < this.bgcolor.length) ReadTools.colorid = id;
        this.SetColor(ReadTools.colorid);

        id = parseInt(Cookie.get('read_fontid'));
...
if (usePages) {
    ...
    //显示翻页提示
    var read_hidetip = Cookie.get('read_hidetip');
    if (read_hidetip != 1) {
        $_('operatetip').style.display = '';
        Cookie.set('read_hidetip', '1', 365, '/', '', '');
    }
} else {
