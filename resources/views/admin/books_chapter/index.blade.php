        ...
        <section class="panel">
            <div class="panel-body">
                <div class="btn-group" style="top: -7px;">
                    <a role="button" class="btn btn-primary" href="{{to_route('admin.chapter.create',['uuid'=>$bookinfo['uuid']])}}">
                        添加章节 <i class="fa fa-plus"></i>
                    </a>
                </div>
                <div class="adv-table" >
                    <table aria-describedby="editable-sample_info" class="table table-striped table-hover table-bordered dataTable">
                        ...
                        @if(isset($chapter))
                            @foreach($chapter as $item)
                                <tr>
                                    <th>{{$item['id']}}</th>
                                    <th><a href="{{to_route('admin.chapter.edit',['id'=>$item['id']])}}" title="查看:{{$item['title']}}内容">{!! $item['title'] !!}</a></th>
                                    <th>{{$item['create_time']}}</th>
                                    <th>{{$item['reading_num']}}</th>
                                    <th>{{$item['url']}}</th>
                                    <th>
                                        <a href="{{to_route('admin.chapter.edit',['id' => $item['id']])}}">
                                            <button class="btn btn-warning btn-xs"><i class="fa fa-pencil"></i> 编辑</button>
                                        </a>
                                        <a class="ajax-restore operation" data-id="{{$item['id']}}" href="javascript:;">
        ...
        $(function(){
            $(".ajax-restore").click(function(){
                ...
                layer.confirm("是否确认操作?",{icon:3}, function () {
                    $.post("{{to_route('admin.chapter.del')}}",{'id':_this.attr('data-id'),"_token":"{{csrf_token()}}"},function (result) {
                        if(result.status == 1){
