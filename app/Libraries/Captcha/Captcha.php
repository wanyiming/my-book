...
class Captcha
{
    ...
    {
        $this->backgrounds = $this->files->files(public_path('captcha_assets/backgrounds'));


        $this->fonts = $this->files->files(public_path('captcha_assets/fonts'));
    ...
    public function check($value)
    {
        ...
        //获取session中的验证码时间，检测验证码是否超时
        $time = $this->session->get('captcha.time');
        //获取超时时间
        $captcha_over_time = config('captcha.captcha_over_time');
        //获取当前时间
        $now_time = time();
        //获取当前时间与生成验证码时间之差
        $diff_time = bcsub($now_time,$time);
        if($diff_time > $captcha_over_time){
            return 10008;
        }
//        $captcha_session = $this->session->get('captcha');
//        print_r($captcha_session);die;
